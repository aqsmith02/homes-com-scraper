"""
unsold_homes_scraper.py

The file uses a function called scrape_5_houses repeatedly to scrape info from unsold homes on homes.com. 
...
def scrape_20_houses(page,eighth):
    """
    ...
    csv_title = f"{page}_{eighth}_homes.csv"
    ...
    BASE_MAIN_LINK = 'https://www.homes.com/buncombe-county-nc/houses-for-sale'
    END_MAIN_LINK = '/?listing_status=5&dom-min=3m'
    MAIN_LINK = f"{BASE_MAIN_LINK}{page_ext}{END_MAIN_LINK}"
    ...
    with open(csv_title, 'w', newline='') as f:
        ...
        with sync_playwright() as p:
            ...
            for i in range(start,end):
                ...
                info1 = placard.locator('ul.detailed-info-container li')
                ...
                try:
                    ...
                    price_only = raw_price.split()[0].strip().replace("$","").replace(",","")
                    price_list.append(int(price_only))
                except:
                    price_list.append(None)

                try:
                    beds = info1.nth(0).inner_text()
                ...
                try:
                    price_per_sq_ft = round(int(price_only)/int(sq_ft_number))
                    price_per_sq_ft_list.append(price_per_sq_ft)
                except:
                ...
                try:
                    year_locator = page.locator('div.subcategory:has(p.amenity-name:has-text("Year Built"))')
                    year_text = year_locator.locator('li.amenities-detail').inner_text().strip()
                    year = year_text.split()[-1]
                    year_built_list.append(int(year))
                except:
                ...
                try:
                    page.wait_for_selector(".search-activity-card")
                    cards = page.query_selector_all(".search-activity-card")
                    for card in cards:
                        title = card.query_selector(".search-activity-card-title")
                        value = card.query_selector(".search-activity-card-text")
                        if title and "Days On Market" in title.inner_text():
                            dom = int(value.inner_text().strip())
...
def scrape_all(page_start,page_end,skip_first_half):
    """
        ...
        for page in range(page_start,page_end):
            ...
            for half in range(2):
                if (skip_first_half) & (page == page_start) & (half==0):
                    csv_titles.append(f"{page_start}_1_homes.csv")
                    csv_titles.append(f"{page_start}_2_homes.csv")
                    csv_titles.append(f"{page_start}_3_homes.csv")
                    csv_titles.append(f"{page_start}_4_homes.csv")
                    continue
                processes = []
                p1 = multiprocessing.Process(target=scrape_20_houses,args=[page,(1+(4*half))])
                processes.append(p1)
                csv_titles.append(f"{page}_{(1+(4*half))}_homes.csv")
                p2 = multiprocessing.Process(target=scrape_20_houses,args=[page,(2+(4*half))])
                processes.append(p2)
                csv_titles.append(f"{page}_{(2+(4*half))}_homes.csv")
                p3 = multiprocessing.Process(target=scrape_20_houses,args=[page,(3+(4*half))])
                processes.append(p3)
                csv_titles.append(f"{page}_{(3+(4*half))}_homes.csv")
                p4 = multiprocessing.Process(target=scrape_20_houses,args=[page,(4+(4*half))])
                processes.append(p4)
                csv_titles.append(f"{page}_{(4+(4*half))}_homes.csv")
                ...
                print(f"Completed step {half+1} for page {page}.")
                time.sleep(120)
                ...
                if half == 1:
                    ...
                    combined_df.to_csv(f"{page}.csv", index=False)
